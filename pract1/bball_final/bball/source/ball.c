...
void ballRender( ball* b, int camera_x, int camera_y )
{
 ...
 if (!b->enabled) {sprite[0] = ATTR0_DISABLED; return;} 
 int x, y;
...
u8 ballCollide(ball* b1, ball* b2) {
   if (!b1->enabled || !b2->enabled) { return 0;}	
    int x1 = b1->x - c_radius, y1 = b1->y - c_radius, x2 = b2->x - c_radius, y2 = b2->y - c_radius;
    
    // Do not overlap on x axis
    if (x1 + (c_diam << 8) < x2 || x1 > x2 + (c_diam << 8))
        return 0;
    
    // Do not overlap on y axis
    if (y1 + (c_diam << 8) < y2 || y1 > y2 + (c_diam << 8))
        return 0;
