...
void resetBall_51Sprites( void ) {
 ...
 for( n = 0; n < NPILOTETES; n++ )  {
  ...
  g_ball[n].sprite_index = n; // tots el mateix

  // use affine matrix 0 (0->31)
  g_ball[n].sprite_affine_index =  (n == PROTA ? 0 : 1);
 ...
 */
 ...
 ballRender( &g_ball[PROTA], 0, 0 ); //ballRender( &g_ball, 0, 0 );        
...
void processLogic( void ) {
 processInput();
 // ballUpdate( &g_ball );
 ballUpdate( &g_ball[PROTA] );
 for(int i = 0; i < NPILOTETES; i++) {
   if (i == PROTA)
    continue;
