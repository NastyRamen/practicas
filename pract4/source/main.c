...
#define SAMPLESPERBUF 8178
#define BYTESPERSAMPLE 1
static u8 mostresMisterioses[] = {
...
//----------------------------------------------------------------------------
	...
	ndspWaveBuf waveBuf;//https://libctru.devkitpro.org/structtag__ndspWaveBuf.html
	...
	u32 *audioBuffer = (u32*)linearAlloc(SAMPLESPERBUF*BYTESPERSAMPLE);//quitamos el *2 ya que no es un array de 2 ya
	...
	ndspChnSetFormat(0, NDSP_FORMAT_MONO_PCM8); //https://libctru.devkitpro.org/channel_8h.html Cambiamos de 16 (2bytes) a 8 (1) 
	...
	memcpy(audioBuffer, mostresMisterioses, SAMPLESPERBUF * BYTESPERSAMPLE);
	...
	while(aptMainLoop()) {
		...
		if (waveBuf.status == NDSP_WBUF_DONE) {
			//Cuando acaba la reproduccion del sonido
